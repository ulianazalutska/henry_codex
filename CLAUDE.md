...
- **Routes**: `app/page.tsx` (home), `app/kolekcje/page.tsx` (collections index), `app/kolekcje/[collection]/page.tsx` (collection template — Atelier/Studio/Lounge), `app/kolekcje/[collection]/[product]/page.tsx` (product template, dynamic for all 14 models), `app/kolekcje/[collection]/inspiracje/page.tsx` (one shared inspiration gallery per collection, not per product — driven by `inspiracje-experience.tsx`; every product page links to its collection's inspiracje page), `app/kontakt/`, `app/personalizacja/`, `app/filozofia-henry/`, `app/faq/`, `app/projekty-indywidualne/` (bespoke/custom projects page, driven by `bespoke-experience.tsx`), `app/blog/` (index) and `app/blog/[slug]/` (post template, dynamic for all entries in `app/blog-data.ts`). The burger menu (`site-navigation.tsx`) also links to `/dla-architektow`, which has no route folder yet — a known gap, not a bug to "fix" by deleting the link unless asked.
...
- **Inspiracje images**: sourced from `collections.inspirationImages?: string[]` in `app/collections-data.ts`, filled in for all three collections (Atelier 9, Studio 9, Lounge 15 photos); real files live in `public/media/inspiracje/<atelier|studio|lounge>/`.
